...
​		感知机模型的基本思想是：先随机选择一个超平面，对样本点进行划分，然后当一个实例点被误分类，即位于分类超平面错误一侧时，调整初始值向量$\mathbf w$ 和偏置常数$\mathbf b$ ，使分类超平面向该误分类点的一侧移动，直至超平面越过该误分类点为止。所以，如果给的初始值不同，则最后得到的分割超平面 $\mathbf{w\cdot x + b} = 0$ 也可能不同，即感知机模型存在多个分割超平面。
